...
gg_heatmap <- function(data, distance, linkage, row_sort=TRUE,
                       labmat = NULL, rlabmat = NULL,
                       colbrew = "Set1", rcolbrew = "Set1") {
    ...
    gg_p <- gg_p +
        annotate("segment",
                 ...
                 y = (p+1)*(1.01) + p/6*gg_hc2$segments$y,
                 yend = (p+1)*(1.01) + p/6*gg_hc2$segments$yend,
                 size = .3)
    if (row_sort) {
        gg_p <- gg_p +
            annotate("segment",
                     ...
                     x = (n+1)*(1.01) + n/6*gg_hc1$segments$y,
                     xend = (n+1)*(1.01) + n/6*gg_hc1$segments$yend,
                     size = .3)
    }
        ...
        for (i in 1:length(labmat)) {
            gg_p <- gg_p +
                annotate("rect",
                         xmin = (1:n) - .5,
                         xmax = (1:n) + .5,
                         ymin = rep(-(i+.5)*lab_width, n),
                         ymax = rep(-(i-.5)*lab_width, n), 
                         fill = setlist1[[i]][1 + labmat[hc2$order, i]],
                         alpha=2/3) #+
        ...
        for (i in 1:length(rlabmat)) {
            ...
            gg_p <- gg_p +
                annotate("rect",
                         ymin = (1:p) - .5,
                         ymax = (1:p) + .5,
                         xmin = rep(-(i+.5)*lab_width2, p),
                         xmax = rep(-(i-.5)*lab_width2, p), 
